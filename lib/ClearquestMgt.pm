...
sub makeQuery {
	my ($typeOfQuery, $fieldsToRetrieve, $filtersToApply, $sortingToApply) = @_;
	...
	DEBUG "Build filter...";
	_processQueryNode($filtersToApply, $querydef);
...
sub sortQuery {
	...
	{ 	ERROR "field \"$field\" cannot be used for sorting because it is not defined" and next unless $a->{$field};
	...
	if($node->{node}) {
		if (ref($node->{node}) eq 'ARRAY') {
			foreach my $node (@{$node->{node}}) {
				_processQueryNode($node, $queryNode);
			}
		}
		else {
			_processQueryNode($node->{node}, $queryNode);
		}
